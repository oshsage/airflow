...
with DAG(
    ...
    schedule='0 7 * * *',
    start_date=pendulum.datetime(2023,4,1, tz='Asiz/Seoul'),
    catchup=False
) as dag:
    '''서울시 코로나19 확진자 발생동향'''
    tb_corona19_count19_status = SeoulApiToCsvOperator(
        task_id='tb_corona19_count_status',
        dataset_nm='TbCorona19CountStatus',
        path='/opt/airflow/files/TbCorona19CountStatus/{{data_interval_end.in_timezone("Asia/Seoul") | ds_nodash }}', # worker node에서 실행되는 것이기 때문에 현재 경로를 wsl과 연결해줘야 함.
    ...
    tb_corona19_count19_status >> tv_corona19_vaccine_start_new
